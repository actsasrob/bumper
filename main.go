...
var provider *ec2rolecreds.EC2RoleProvider
	...
	for {
		...
		if (req.Method == "PUT" && !strings.Contains(req.RequestURI, "partNumber")) ||
		   (req.Method == "POST" && strings.HasSuffix(req.RequestURI, "uploads")) {
			...
			// Use alternate path get get EC2 instance role credentials
			// Create a Session with a custom region
			if awsconfig == nil {
				awsconfig = &aws.Config{
        			HTTPClient: &http.Client{Timeout: 10 * time.Second},
        			Region: aws.String(bumper.awsregion),
				}
			}
			...
			if provider == nil {
				provider = &ec2rolecreds.EC2RoleProvider{
    					...
    					Client: ec2metadata.New(sess, awsconfig),
			...
			if creds == nil {
	                ...
	                //if err != nil {
		        //   log.Printf("error retrieving aws credentials, got %v", err)
		        //   SendResp(cli, cliwriter, 500, err.Error(), false)
		        //   return
                        //}
                        //fmt.Println("info: aws credentials %s", credsValue)
	                signer := v4.NewSigner(creds, func(s *v4.Signer) {
		                     s.DisableURIPathEscaping = true
                                     s.DisableRequestBodyOverwrite = true
                                  })
                        req.Header.Add("X-Amz-Server-Side-Encryption", "aws:kms")
                        req.Header.Add("X-Amz-Server-Side-Encryption-Aws-Kms-Key-Id", bumper.kmskeyid)
                        _, err = signer.Sign(req, nil, "s3", bumper.awsregion, epochTime())
	                if err != nil {
		           log.Printf("aws v4 signer expect no error, got %v", err)
		           SendResp(cli, cliwriter, 500, err.Error(), false)
		           return
                        }
                }
...
//
var opts struct {
	AwsRegion     string `short:"r" long:"awsregion" value-name:"<awsregion>" description:"AWS Region" `
	...
	KmsKeyId       string `short:"i" long:"kmskeyid" value-name:"<arn:aws:kms:region:account:key>" description:"The KMS customer master encryption key ID" required:"true"`
...
func main() {
	...
	bumper.timeout = opts.Timeout
        ...
        if opts.AwsRegion != "" {
        	bumper.awsregion = opts.AwsRegion
	} 
